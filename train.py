...
def train(y_group, epochs, img_height, img_width, batch_size, checkpoint_path_load, path_img_train,
    ...
    if not eval_only:
        ...
        traingen = CustomDataGen(y_group, path_img_train, batch_size, use_bool=False, resize=True, height=img_height,
                                 width=img_width)
        ...
        print('evaluation\n')
        testgen = CustomDataGen(y_group, path_img_test, batch_size, use_bool=False, resize=True, height=img_height,
                                width=img_width)
        loss, dice_score = unet.evaluate(testgen)
        print('loss on test images = {}\n'.format(loss))
        print('dice score on test images = {}\n'.format(dice_score))
        return unet, model_history.history, loss, dice_score  # loss and dice_score are evaluated on the test data
        ...
        print('loss on test images = {}\n'.format(loss))
        print('dice score on test images = {}\n'.format(dice_score))
        return unet, loss, dice_score  # loss and dice_score are evaluated on the test data
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_height', type=int, default=128,
                        help='images will be rescaled to such height, should be a power of two')
    parser.add_argument('--img_width', type=int, default=128,
                        help='images will be rescaled to such width, should be a power of two')
    ...
    parser.add_argument('--path_img_train', type=str,
                        default="small-subset-of-airbus-ship-segmentation-dataset/train_v2/",
                        help='from here will be generated batches of images for training')
    parser.add_argument('--path_img_test', type=str,
                        default="small-subset-of-airbus-ship-segmentation-dataset/test_v2/",
    ...
    parser.add_argument('--save_plot', action='store_true',
                        help='whether to save graphs of the training loss and metric')
    parser.add_argument('--eval_only', action='store_true', help='whether to evaluate model without training')
    parser.add_argument('--path_y_train', type=str,
                        default="./small-subset-of-airbus-ship-segmentation-dataset/train_ship_segmentations_v2.csv",
    ...
    # call train() with all arguments;
    # it will return the model, history of training loss and metric, loss and dice score on the test data
    unet, hist, loss, dice_score = train(y_group, args.epochs, args.img_height, args.img_width, args.batch_size,
                                         args.checkpoint_path_load,
                                         args.path_img_train, args.path_img_test, args.checkpoint_path_save,
                                         args.save_weights, args.use_pretrained, args.save_plot, args.eval_only)
